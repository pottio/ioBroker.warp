...
class WarpApiDefinitions {
  ...
  defineEvse() {
    ...
    evse.add("evse/dc_fault_current_state", "The state of the DC fault current protection module. If a DC fault occurs, charging is no longer possible until the protection module has been reset. Before resetting, it is imperative that the reason for the fault is rectified!", [
      ...
      import_models.Param.butt("reset_dc_fault_current", "normal").onlyWarp2().withDescription("Resets the DC residual current protection module. Before resetting, it is imperative that the reason for the fault is rectified!").actionSendCommand("evse/reset_dc_fault_current", `{ "password": "0xDC42FA23" }`).build()
    ]);
    ...
    evse.add("evse/manual_charging", "Allows to start and stop a charging process, when auto start charging is disabled.", [
      ...
      import_models.Param.butt("stop_charging", "stop").withDescription("Stops a charging process").actionSendCommand("evse/start_charging").build()
    ]);
    return evse;
  }
  defineMeter() {
  ...
  defineOthers() {
    ...
    const esp = new import_models.WarpApi("esp", "", true);
    esp.add("esp", "ESP controlling", [
      import_models.Param.butt("reboot").withDescription("Restarts the ESP to apply configuration changes, for example").actionSendCommand("reboot").build()
    ]);
